...
class Oauth2LoginFlow(LoginFlow[OidcAuthProvider]):
    ...
    async def async_step_auth_request(
        ...
        try:
            ...
            raise OidcError("unable to determine return callback URL") from err
        ...
        try:
            ...
            raise OidcError(f"invalid OIDC discovery data: {err}") from err
        ...
        for key in url_keys:
            ...
            if urllib.parse.urlparse(url).scheme != "https":
                raise OidcError(f"{key} is not an HTTPS url: {url}")
